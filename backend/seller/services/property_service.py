...
class PropertyService(BaseService):
    ...
    def _enrich_with_ai_predictions(self, data: dict) -> dict:
        ...
        try:
            ...
            area = float(data.get('area_sqft', 1000) or 1000)
            price = float(data.get('price', 0) or 0)
            rate = float(data.get('rate_per_sqft', 0) or 0)
            if rate <= 0 and price > 0 and area > 0:
                rate = price / area

            conditions = {
                'locality': data.get('locality') or data.get('address') or 'Science City',
                'property_type': data.get('property_type') or 'flat',
                'bhk': int(data.get('bhk', 2) or 2),
                'area_sqft': area,
                'rate_per_sqft': rate,
                'price': price,
                'latitude': float(data.get('latitude', 23.0225) or 23.0225),
                'longitude': float(data.get('longitude', 72.5714) or 72.5714),
                'reconstruction_needed': data.get('reconstruction_needed', ''),
                'amenities': amenity_names,
                'builder_name': data.get('builder_name', ''),
                'property_age': int(data.get('property_age', 1) or 1),
            }
            res = PredictionService().predict_by_conditions(conditions)
            data['predicted_price'] = res.get('predicted_price')
            data['base_ml_price'] = res.get('base_ml_price')
            data['amenity_adjustment'] = res.get('facility_adjustment')
            ...
            data['investment_score'] = res.get('investment_score')
            data['investment_rating'] = res.get('investment_rating')
            data['investment_explanation'] = res.get('investment_explanation')
    ...
    def update_property(self, property_id: str, data: dict, seller_id: str = None) -> dict:
        ...
        # Prediction inputs must be compared against the persisted complete
        # listing.  Previously this method predicted from the PATCH-like
        # update payload alone, which silently replaced missing inputs with
        # defaults and changed an unchanged listing's suggested price.
