  ...
  return (
    <style>{`
      ...
      .bl-btn-secondary .bl-play{
        ...
        .bl-navlink:hover { background: rgba(0,0,0,0.05); }
        .bl-navlink-active {
          background: #fff;
          color: var(--ink);
  ...
  return (
    <nav className="bl-navbar-floating bl-glass" style={{ borderRadius: 999, padding: "10px 10px 10px 22px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="bl-navlinks-pill bl-nav-links">
          {navItems.map((item) => {
            ...
            return (
              <a
                ...
                className={isActive ? "bl-navlink bl-navlink-active" : "bl-navlink"}
              >
                {isActive && <span className="bl-dot" style={{ marginRight: 6 }} />}
                {item.label}
              </a>
            );
          })}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={() => onNavigate('signup')} className="bl-btn-primary bl-nav-links" style={{ borderRadius: 999 }}>
      ...
      {open && (
        <div className="bl-mobile-menu" style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 16, padding: "16px 8px 8px", borderTop: `1px solid ${C.border}`, animation: "ax-fadeup 0.3s ease" }}>
          {navItems.map((item) => (
            <a key={item.id} href={`#${item.id}`} onClick={(e) => { e.preventDefault(); scrollTo(item.id); }} style={{ color: C.text, textDecoration: "none", fontSize: 15, fontWeight: 500, opacity: 0.8, padding: "8px 0" }}>{item.label}</a>
          ))}
          ...
          <button onClick={() => { setOpen(false); onNavigate('signup'); }} className="bl-btn-primary" style={{ justifyContent: "center", borderRadius: 999 }}>Get started</button>
        </div>
      )}
    </nav>
  );
