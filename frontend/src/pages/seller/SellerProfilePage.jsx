    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
          ...
          <span>Seller Profile Management</span>
        </h1>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-card-soft space-y-6">
        <div className="flex items-center space-x-4 border-b border-slate-100 pb-6">
          <div className="w-16 h-16 rounded-full bg-blue-600 flex items-center justify-center text-white text-2xl font-extrabold shadow-md shadow-blue-500/20">
            {formData.first_name ? formData.first_name[0] : (user?.full_name ? user.full_name[0] : 'S')}
          </div>
          <div>
            <h2 className="text-lg font-bold text-slate-900">
              {user?.full_name || `${formData.first_name} ${formData.last_name}`.trim() || 'Seller Account'}
            </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="pt-4 flex justify-end">
            <button
              type="submit"
              className="inline-flex items-center space-x-2 px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm shadow-md shadow-blue-500/20 transition-all cursor-pointer"
            >
